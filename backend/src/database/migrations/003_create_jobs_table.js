...
export async function up(knex) {
    return knex.schema.createTable('jobs', (table) => {
        ...
        // GIN indexes for JSONB fields (Spécifique Postgres/Supabase)
        table.index('skills_required', 'skills_required_gin', 'GIN');
        table.index('skills_preferred', 'skills_preferred_gin', 'GIN');
        table.index('screening_questions', 'screening_questions_gin', 'GIN');
    }).then(() => {
        // Index de recherche textuelle (Full-Text Search) pour Postgres
        // On crée un index GIN sur un tsvector combinant le titre et la description
        return knex.raw(`
