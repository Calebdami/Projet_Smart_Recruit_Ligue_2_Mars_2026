  ...
  <transition name="modal-backdrop">
    ...
    >
      ...
      <transition name="modal-content">
        <div
          v-if="show"
          class="relative max-w-md w-full transform rounded-2xl border border-slate-200/90 bg-white p-8 shadow-2xl dark:border-slate-700 dark:bg-black"
          @click.stop
        >
          <!-- Icon Circle -->
          ...
          <div class="flex flex-col-reverse sm:flex-row gap-3 justify-center">
            <button
              ...
              @click="handleCancel"
              :disabled="loading"
              class="rounded-xl px-6 py-2.5 text-sm font-medium text-slate-700 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-black dark:text-slate-200 dark:hover:bg-slate-900 dark:focus:ring-offset-black bg-slate-100 hover:bg-slate-200"
            >
              {{ cancelText }}
            </button>
            <button
